...
const createTruck = (truckData) => __awaiter(void 0, void 0, void 0, function* () {
    if (!truckData) {
        throw new Error('truckData was not properly passed to the repository!');
    }
    const truckModel = modelToCreateTruck();
    try {
        const newTruck = new truckModel(Object.assign(Object.assign({}, truckData), { isBlocked: false, isVerified: false, status: 'active', createdAt: new Date(), expiresAt: new Date() }));
        // Save the new truck to the database
        const doneTheJob = yield newTruck.save();
        if (!doneTheJob)
            return false;
